...
Use coderef's existing `vX.Y.Z` tags and keep all public package versions consistent.
Build and checksum all four platform bundles before publishing. Assemble and verify assets on a
mutable draft, then publish once; never clobber assets on a published immutable release. Retry npm
and Marketplace publishing independently in the documented GitHub → npm → Marketplace order.
Never delete or move published tags as a routine recovery; publish a corrected version instead.
